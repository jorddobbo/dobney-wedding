...
function rsvp_frontend_prompt_to_edit($attendee) {
  ...
  $prompt = RSVP_START_CONTAINER."<div class='rsvp__form--added'>";
  $editGreeting = __("Hi %s it looks like you have already RSVP'd. Would you like to edit your reservation?", 'rsvp-plugin');
	...
	$prompt .= "<form class=\"rsvp__form--edit\" method=\"post\" action=\"$rsvp_form_action\">\r\n
								<input type=\"hidden\" name=\"attendeeID\" value=\"".$attendee->id."\" />
								<input type=\"hidden\" name=\"rsvpStep\" id=\"rsvpStep\" value=\"editattendee\" />
								<input class=\"btn\" type=\"submit\" value=\"".__("Yes", 'rsvp-plugin')."\" onclick=\"document.getElementById('rsvpStep').value='editattendee';\" />
								<input class=\"btn\" type=\"submit\" value=\"".__("No", 'rsvp-plugin')."\" onclick=\"document.getElementById('rsvpStep').value='newsearch';\"  />
							</form>\r\n";
  $prompt .= "</div>".RSVP_END_CONTAINER;
	return $prompt;
}
...
function rsvp_frontend_main_form($attendeeID, $rsvpStep = "handleRsvp") {
	...
	$form = "<form class=\"rsvp__form--add\" id=\"rsvpForm\" name=\"rsvpForm\" method=\"post\" action=\"$rsvp_form_action\" autocomplete=\"off\">";
  ...
  if($attendeeID <= 0) {
	...
	$form .= "<div class=\"rsvp__form-radio\">".

    rsvp_BeginningFormField("", "").
    "<input type=\"radio\" name=\"mainRsvp\" value=\"Y\" id=\"mainRsvpY\" ".((($attendee->rsvpStatus == "No") || ($rsvp_saved_form_vars['mainRsvp'] == "N")) ? "" : "checked=\"checked\"")." /> <label for=\"mainRsvpY\">"."Yes – <i>with pleasure</i>"."</label>".
    RSVP_END_FORM_FIELD.
    rsvp_BeginningFormField("", "").
      ...
      "<label for=\"mainRsvpN\">"."No – <i>with regret</i>"."</label>".
    RSVP_END_FORM_FIELD.
  ...
  if(get_option(OPTION_RSVP_HIDE_EMAIL_FIELD) != "Y") {
	...
	if(get_option(RSVP_OPTION_HIDE_NOTE) != "Y") {
  	$form .= RSVP_START_PARA."<h3><span>Dietary Requirements</span></h3>".RSVP_END_PARA.
      rsvp_BeginningFormField("", "").
      ...
      if($a->id != $attendeeID) {
        ...
        $form .= "<div class=\"rsvpAdditionalAttendeeQuestions\">\r\n";
  			$form .= rsvp_BeginningFormField("", "").RSVP_START_PARA.sprintf(__(" Will %s be attending?", 'rsvp-plugin'), esc_html(stripslashes($a->firstName." ".$a->lastName))).RSVP_END_PARA.
                "<div class=\"rsvp__form-radio\"><div class=\"rsvpFormField\"><input type=\"radio\" name=\"attending".$a->id."\" value=\"Y\" id=\"attending".$a->id."Y\" /> ".
                "<label for=\"attending".$a->id."Y\">$yesText</label></div>
  							<div class=\"rsvpFormField\"><input type=\"radio\" name=\"attending".$a->id."\" value=\"N\" id=\"attending".$a->id."N\" /> ".
                "<label for=\"attending".$a->id."N\">$noText</label></div></div>".
                RSVP_END_FORM_FIELD;
	...
	$form .= RSVP_START_PARA."<input class=\"btn\" type=\"submit\" value=\"".__("RSVP", 'rsvp-plugin')."\" />".RSVP_END_PARA;
  ...
  $questions = $wpdb->get_results($sql);
	if(count($questions) > 0) {
		...
		foreach($questions as $q) {
			...
			$output .= rsvp_BeginningFormField("", "")."<h3><span>".stripslashes($q->question)."</span></h3>";
...
function rsvp_find(&$output, &$text) {
	...
	if($attendee != null) {
		...
		if(strtolower($attendee->rsvpStatus) == "noresponse") {
			...
			if(trim(get_option(OPTION_WELCOME_TEXT)) != "") {
				...
				$output .= "<div class='rsvp__form__message'>".__("There are a few more questions we need to ask you if you could please fill them out below to finish up the RSVP process.", 'rsvp-plugin')."</div></div>";
			}
  ...
  if(rsvp_require_only_passcode_to_register()) {
    $notFoundText = sprintf(RSVP_START_PARA.__('<div class="rsvp__form--errored">We were unable to find anyone with the password you specified.</div>', 'rsvp-plugin').RSVP_END_PARA);
  } else if (rsvp_require_passcode()) {
    $notFoundText = sprintf(RSVP_START_PARA.__('<div class="rsvp__form--errored">We were unable to find anyone with a name of %1$s %2$s or the provided password was incorrect.</div>', 'rsvp-plugin').RSVP_END_PARA, htmlspecialchars($firstName), htmlspecialchars($lastName));
  } else {
    $notFoundText = sprintf(RSVP_START_PARA.__('<div class="rsvp__form--errored">We were unable to find anyone with a name of %1$s %2$s.</div>', 'rsvp-plugin').RSVP_END_PARA, htmlspecialchars($firstName), htmlspecialchars($lastName));
  }
...
function frontend_rsvp_thankyou($thankYouPrimary, $thankYouAssociated) {
    ...
    if(count($thankYouAssociated) > 0) {
      ...
      $tyText = rtrim(trim($tyText), ",").".";
		...
		return RSVP_START_CONTAINER."<div class='rsvp__form--thank-you'>".$tyText."</div>".RSVP_END_CONTAINER;
	}
}
...
function rsvp_frontend_greeting() {
  ...
  if(get_option(OPTION_RSVP_OPEN_REGISTRATION) == "Y") {
	...
	$output .= "<form class=\"rsvp__form--standard\" name=\"rsvp\" method=\"post\" id=\"rsvp\" action=\"$rsvp_form_action\" autocomplete=\"off\">\r\n";
	$output .= "	<input type=\"hidden\" name=\"rsvpStep\" value=\"find\" />";
  if(!rsvp_require_only_passcode_to_register()) {
	$output .= "<div class='rsvp__form__input'><input type=\"text\" name=\"firstName\" id=\"firstName\" size=\"30\" value=\"\" class=\"required\" placeholder=\"First Name\" /></div>";
	$output .= "<div class='rsvp__form__input'><input type=\"text\" name=\"lastName\" id=\"lastName\" size=\"30\" value=\"\" class=\"required\" placeholder=\"Last Name\" /></div>";
  }
	$output .= "<input class=\"rsvp__find-button btn\" type=\"submit\" value=\"".__("Find Me", 'rsvp-plugin')."\" />";
	$output .= "</form>\r\n";
	$output .= RSVP_END_CONTAINER;
	return $output;
}
