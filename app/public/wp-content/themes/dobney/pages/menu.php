<?php 

/*

Template Name: Menu

*/

?>

<div class="dbo dbo--menu">

	<?php get_header(); ?>

		<div class="dbo__title wow fadeInUp">

			<h1>Menu</h1>

			<div class="dbo__container">
				<img src="<?php echo get_template_directory_uri(); ?>/assets/img/menu/menu_main.png" />
			</div>
		</div>

		<div class="dbo--menu__content">
			<div class="dbo__container wow fadeInUp">
				<p class="dbo--menu__intro">Integer pellentesque libero at neque interdum, at finibus nisi vestibulum. Quisque id porttitor urna. Cras condimentum ex eu nulla sagittis, id consequat massa convallis. Nulla justo dolor.</p>
				<ul class="dbo--menu__section canapes">
					<li class="title"><h3>Canapes</h3></li>
					<li>Lincolnshire poacher cheese spheres (v)</li>
					<li>Sour dough bruschetta, tomato and basil chutney (v)</li>
					<li>Mini fish n chips</li>
					<li>Lincolnshire sausages, port and grain mustard glaze</li>
				</ul>
				<div class="dbo__divider">
					<img src="<?php echo get_template_directory_uri(); ?>/assets/img/leaf-sm.svg" />
				</div>
				<ul class="dbo--menu__section mains">
					<li class="title"><h3>Mains (Sharing)</h3></li>
					<li>Roasted rib eye steak, red onion and thyme marmalade</li>
					<li>Crispy chicken parcel, harissa dip</li>
					<li>Sun blushed tomato, mozzarella and basil risotto cake (v)</li>
					<li class="spacer"></li>
					<li>Garlic & thyme new potatoes</li>
					<li>Hot spring vegetable salad, mint oil (sugar snaps, green beans, peas & courgette)</li>
					<li>Red cabbage, celeriac and shallot coleslaw</li>
				</ul>
				<div class="dbo__divider">
					<img src="<?php echo get_template_directory_uri(); ?>/assets/img/leaf-sm.svg" />
				</div>
				<ul class="dbo--menu__section desserts">
					<li class="title"><h3>Kids</h3></li>
					<li>Chicken goujons & chips (tomato ketchup) + Dessert</li>
				</ul>
				<div class="dbo__divider">
					<img src="<?php echo get_template_directory_uri(); ?>/assets/img/leaf-sm.svg" />
				</div>
				<ul class="dbo--menu__section drinks">
					<li class="title"><h3>After Dinner</h3></li>
					<li>Coffee & Tea with artisan chocolates</li>
				</ul>
				<div class="dbo__divider">
					<img src="<?php echo get_template_directory_uri(); ?>/assets/img/leaf-sm.svg" />
				</div>
				<ul class="dbo--menu__section drinks">
					<li class="title"><h3>Evening Food</h3></li>
					<li>Roasted chicken, red chilli, yellow pepper and rocket pizza</li>
					<li>Sun blushed tomato, mozzarella and baby basil pizza (v)</li>
				</ul>
				<div class="dbo__divider">
					<img src="<?php echo get_template_directory_uri(); ?>/assets/img/leaf-sm.svg" />
				</div>
				<ul class="dbo--menu__section drinks">
					<li class="title"><h3>Evening Food</h3></li>
					<li>Dark chocolate brownie, honeycomb</li>
					<li>Lemon zest cheesecake</li>
					<li>Eton mess parfait</li>
				</ul>
				<div class="dbo__divider">
					<img src="<?php echo get_template_directory_uri(); ?>/assets/img/leaf-sm.svg" />
				</div>
			</div>
		</div>

	</div>

	<?php get_footer(); ?>

</div>